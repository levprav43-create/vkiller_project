...
# ========================================================================================================================================
                ...
                # ГЛАВНАЯ Я ДУМАЮ ПРОБЛЕМА СЕЙЧАС В ТОМ ЧТО БОТ ОТ СВОЕГО КАНАЛА НЕ МОЖЕТ ВЕСТИ ПОИСК
                # МОЖЕТ КАК ТО ОРГАНИЗОВАТЬ ПОИСК ПО ОДНОМУ ИЗ НАШИХ ID ИЛИ ПОЛУЧИТЬ ДОСТУП ДЛЯ БОТА
                        ...
                        with Session(engine) as db_session:
                            try:
                                ...
                                raw_candidates = vk_client.search_users(
                                    ...
                                    city=2,   # ВОТ ЗДЕСЬ ОКАЗАЛОСЬ ЧИСЛО НУЖНО
                                    count=10
                                )
                                # filtered = filter_candidates(raw_candidates, blacklist_vk_ids, favorites_vk_ids)

                                from pprint import pprint
                                pprint(raw_candidates)   # ОТВЕТ ОТ API

                                if raw_candidates:
                                    candidate = raw_candidates[0] # СЧЕТЧИК БЫ ОРГАНИЗОВАТЬ В ПЕРЕМЕННУЮ ЧТОБЫ МОЖНО БЫЛО ПЕРЕМЕЩАТЬСЯ ПО КАНДИДАТАМ ПО ИНДЕКСУ
                                    response = (   #  А ТУТ ЧЕРЕЗ GET ЛУЧШЕ ОБРАЩАТЬСЯ А ТО МОЖЕТ И НЕ БЫТЬ ТАКОГО СОДЕРЖИМОГО В СЛОВАРЕ
                                        f"👤 {candidate['first_name']} {candidate['last_name']}, {f"{candidate.get('age')} лет"}\n"
                                        f"📍 {candidate.get('city', 'Город не указан')}\n"
                                        f"🔗 {candidate.get('profile_url')}\n\n"
                                        f"📸 Фото: {candidate.get('photo_1')}\n\n"
